...
#define CUSTOMFVF (D3DFVF_XYZRHW | D3DFVF_DIFFUSE)
    ...
    LPDIRECT3DVERTEXBUFFER9 v_buffer = null;    // create three vertices using the CUSTOMVERTEX struct built earlier

    struct state_entry
    {
        D3DRENDERSTATETYPE state_id;
        DWORD new_value;
        DWORD old_value;
    };
    ...
    void push_state(LPDIRECT3DDEVICE9 device9)
    {
        for(int i = 0; i < _countof(state); ++i) {
            device9->GetRenderState((D3DRENDERSTATETYPE)state[i].state_id, &state[i].old_value);
            device9->SetRenderState((D3DRENDERSTATETYPE)state[i].state_id, state[i].new_value);
        }
    }

    void pop_state(LPDIRECT3DDEVICE9 device9)
    {
        for(int i = 0; i < _countof(state); ++i) {
            device9->SetRenderState((D3DRENDERSTATETYPE)state[i].state_id, state[i].old_value);
        }
    }

    void draw_triangle(LPDIRECT3DDEVICE9 device9)
    {
        if(v_buffer == null) {
            device9->CreateVertexBuffer(_countof(vertices) * sizeof(CUSTOMVERTEX), 0, CUSTOMFVF, D3DPOOL_MANAGED, &v_buffer, NULL);
            VOID *pVoid;
        ...
        HRESULT hr = device9->Clear(1, reinterpret_cast<D3DRECT const *>(&fill_rect), D3DCLEAR_TARGET, fill_color, 0, 0);    // ARGB
        if(FAILED(hr)) {
            warn(warned, "pDevice->Clear failed: %08x", hr);
        }
        push_state(device9);
        device9->BeginScene();
        device9->SetPixelShader(null);
        device9->SetVertexShader(null);
        device9->SetFVF(CUSTOMFVF);
        device9->SetStreamSource(0, v_buffer, 0, sizeof(CUSTOMVERTEX));
        device9->DrawPrimitive(D3DPT_TRIANGLELIST, 0, _countof(vertices) / 3);
        device9->EndScene();
        pop_state(device9);
    }
    ...
    bool setup_hook()
    {
        ...
        uint16 machine;
        uint16 native_machine;
        bool is_64 = IsWow64Process2(hooked_process, &machine, &native_machine);
        log("WOW: %d,%d,%d", is_64, machine, native_machine);
...
#pragma pack(pop)
    ...
    int len = _vsnprintf_s(msg.buffer, _countof(msg.buffer) - 3, text, v);
    if(len < 0) {
        return;
    }
    char *c = msg.buffer + len;
    if(*c != '\r' && *c != '\n') {
        *c++ = '\r';
        *c++ = '\n';
        *c++ = 0;
